...
| **LightGBM (Statcast)** | **58 特徴量 + Optuna 50 trial（Brier 0.1544、MLB 公式比 +1.90%）** |
| **Bayesian Hierarchical** | NumPyro SVI — チーム力・球場・時代効果の階層モデル + **90% 信用区間** |
...
### Bayesian Hierarchical Model（新規）
...
- **Bayesian Credible Interval**: 90% CI on WP — uncertainty widens in high-leverage, close-game situations
...
- **LightGBM (Statcast)**: 58 特徴量、Brier 0.1544（MLB 公式比 +1.90%）
- **Bayesian Hierarchical**: チーム力・球場・時代効果 + 90% 信用区間
...
|-------|---------|-------|-----------|-------|
| MLB 公式 `home_win_exp` | 0.1574 | 0.3706 | 0.4688 | 0.0153 |
| **LightGBM (Statcast)** | **0.1544** | **0.3825** | **0.4610** | **0.0067** |
| vs MLB 公式 | **+1.90%** | | | |

58 特徴量（投球速度・変化量・打球速度・角度・xwOBA・パークファクター等）、Optuna 50 trial。MLB Stats API live feed からリアルタイム取得可能。
...
| **LightGBM (Statcast)** | 勾配ブースティング（58 特徴量、Optuna 50 trial） | **Brier 0.1544（MLB 公式 0.1574 比 +1.90%）、リアルタイム対応** |
| **Bayesian Hierarchical** | NumPyro SVI 階層モデル（チーム・球場・時代・レバレッジ） | **90% 信用区間付き WP、チーム別強度推定** |
...
│     ↓                                                         │
│   train_wp_bayesian.py (NumPyro SVI)                          │
│     ↓                                                         │
│   Bayesian Hierarchical (team/park/season/leverage)          ├→ 本番WP
...
- [x] 58 特徴量エンジニアリング（投球・打球・バットトラッキング・RE24・パークファクター）
- [x] GitHub Actions ワークフロー（`Train Statcast WP Model`）
- [x] 全カラム欠損ガード（`_safe_col()` ヘルパー）
- [x] **LightGBM Optuna 50 trial 完了（Brier 0.1544、MLB 公式比 +1.90%）**
