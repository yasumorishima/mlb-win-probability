...
| **LightGBM (Statcast)** | **58 特徴量 + Optuna 50 trial（Brier 0.1544、ベンチマーク比 +1.90%）** |
...
Statcast データには投球速度・変化量・打球速度・発射角度・xwOBA・MLB ベンチマーク WP 等 70+ 列を含む。WP モデル学習時は**打席結果のみ（`events IS NOT NULL`）**に絞り、約 172 万行で学習。
...
| **LightGBM (Statcast)** | 勾配ブースティング（58 特徴量、Optuna 50 trial） | **Brier 0.1544（MLB ベンチマーク 0.1574 比 +1.90%）、リアルタイム対応** |
