...
| **LightGBM (Statcast)** | **76 特徴量 + Optuna 50 trial（Brier 0.1544、ベンチマーク比 +1.90%）** |
...
- **Bayesian stacking**: LightGBM が投球・打球・バットトラッキング・選手年齢・ストライクゾーン等 76 特徴量を処理、ベイズ層がチーム力・球場・時代効果 + 不確実性を上乗せ
...
76 特徴量（投球速度・変化量・打球速度・角度・xwOBA・バットトラッキング・選手年齢・ストライクゾーン・リリースポイント・投球軌道・パークファクター等）、Optuna 50 trial。MLB Stats API live feed からリアルタイム取得可能。
...
| **LightGBM (Statcast)** | 勾配ブースティング（76 特徴量、Optuna 50 trial） | **Brier 0.1544（MLB ベンチマーク 0.1574 比 +1.90%）、リアルタイム対応** |
...
│   LightGBM(Statcast, 76 features, Optuna)
│     ↓
...
- [x] **76 特徴量エンジニアリング**（投球・打球・バットトラッキング・選手年齢・ストライクゾーン・リリースポイント・投球軌道・パークファクター）
...
- [ ] **76 特徴量 Statcast + Bayesian + Ensemble 再学習結果待ち**
